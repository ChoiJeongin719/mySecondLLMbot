...
if "messages" not in st.session_state:
    st.session_state.messages = [{"role": "assistant", "content": "안녕하세요! 토론 채팅방입니다.", "type": "system"}]
...
if "max_turns" not in st.session_state:
    st.session_state.max_turns = 5  # 기본값 5턴 설정

if "chat_completed" not in st.session_state:
    st.session_state.chat_completed = False

# Initialize session state for starter topic
if "starter_selected" not in st.session_state:
    st.session_state.starter_selected = False

# Initialize session state for starting with a topic
...
with chat_container:
    ...
    if len(st.session_state.messages) == 1 and not st.session_state.starter_selected:
        st.markdown("""
        <div style='text-align: center; margin: 50px 0 30px 0;'>
            <h3>아래 버튼을 클릭하여 토론을 시작해보세요</h3>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not st.session_state.chat_completed:
        ...
        st.success("토론이 완료되었습니다. 채팅을 초기화하려면 사이드바의 '채팅 초기화' 버튼을 클릭하세요.")
...
if not st.session_state.chat_completed:
    ...
    st.markdown(
        ...
        "토론이 완료되었습니다. 새로운 토론을 시작하려면 사이드바의 '채팅 초기화' 버튼을 클릭하세요."
        "</div>",
        unsafe_allow_html=True
    )
